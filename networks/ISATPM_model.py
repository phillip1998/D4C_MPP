...
class network(nn.Module):
    ...
    def loss_fn(self, scores, targets):
        ...
        return torch.mean(torch.square(scores-targets))+\
                (self.p_score_ms+self.n_score_ms)*0.09 +\
                (torch.abs(0.17-self.p_score_var)+torch.abs(0.17-self.n_score_var))*0.03 +\
                (torch.abs(0.5-self.p_score_mean)+torch.abs(0.5-self.n_score_mean))*0.03
