...
class ISATconvolution_PM(nn.Module):
    ...
    def forward(self, graph, r_node, r2r_edge, i_node, d2d_edge, **kargs):
        ...
        score_P= self.d2r(graph, score_P)

        r_node_P=score_P*r_node
        ...
        score_M= self.d2r(graph, score_M)

        r_node_M=score_M*r_node

        if kargs.get('get_feature',False):
            return score_P_temp, score_M_temp, score_P, score_M
